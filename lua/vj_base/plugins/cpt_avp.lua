...
VJ.AddConVar("vj_avp_survival_kseries",0,bit.bor(FCVAR_ARCHIVE,FCVAR_NOTIFY))
...
VJ.AddNPC("UD-6B \"Mohawk\" Dropship","npc_vj_avp_hum_dropship",vCat_M)
...
if CLIENT then
	...
	hook.Add("PopulateToolMenu", "VJ_ADDTOMENU_AVP", function()
		...
		spawnmenu.AddToolMenuOption("DrVrej", "SNPC Configures", "Aliens vs Predator", "Aliens vs Predator", "", "", function(Panel)
			...
			controlAdd("Checkbox", {Label = "Enable Fatalities", Command = "vj_avp_fatalities"}):SetTextColor(textCol)
			controlAdd("Checkbox", {Label = "Only Attempt Fatalities on Low HP Targets", Command = "vj_avp_fatalities_lowhp"}):SetTextColor(textCol)
			controlAdd("Checkbox", {Label = "Enable Ambience Music [Survival]", Command = "vj_avp_survival_music"}):SetTextColor(textCol)
			controlAdd("Checkbox", {Label = "Always K-Series Xenomorphs [Survival]", Command = "vj_avp_survival_kseries"}):SetTextColor(textCol)
			controlAdd("Checkbox", {Label = "Enable Bots [Survival]", Command = "vj_avp_survival_bots"}):SetTextColor(textCol)
			local p = controlAdd("Slider", {Label = "", min = 0, max = 8, Command = "vj_avp_survival_maxbots"})
			p.Paint = function(w,h)
				surface.SetDrawColor(textCol)
				surface.SetTextPos(0,p:GetTall() *0.35)
				surface.SetTextColor(textCol)
				surface.DrawText("Bot Count (0 = Auto) [Survival]")
			end
			controlAdd("Checkbox", {Label = "Use VJ Players As Bots", Command = "vj_avp_survival_plybots"}):SetTextColor(textCol)
			...
			Panel:AddPanel(vj_icon)
			Panel:AddControl("Label", {Text = "Xenomorph Settings"}):SetTextColor(textCol)
			...
			Panel:AddPanel(vj_icon)
			Panel:AddControl("Label", {Text = "Predator Settings"}):SetTextColor(textCol)
			controlAdd("Checkbox", {Label = "Enable Boss Themes", Command = "vj_avp_bosstheme_p"}):SetTextColor(textCol)
			controlAdd("Checkbox", {Label = "Enable Unique Spawns", Command = "vj_avp_predmobile"}):SetTextColor(textCol)
			controlAdd("Checkbox", {Label = "Enable Predator HUD Info Display", Command = "vj_avp_hud_predinfo"}):SetTextColor(textCol)
			if LocalPlayer():IsAdmin() then
				controlAdd("Checkbox", {Label = "Toggle Predator HUD Info Code", Command = "vj_avp_pred_info"}):SetTextColor(textCol)
				Panel:AddControl("Label", {Text = "Admin Only - Disables the background code for the info display. WILL BREAK THE HUD IN MOST INSTANCES!"}):SetTextColor(textCol)
			end
			...
			Panel:AddPanel(vj_icon)
			Panel:AddControl("Label", {Text = "Human Settings"}):SetTextColor(textCol)
			controlAdd("Checkbox", {Label = "Enable Boss Themes", Command = "vj_avp_bosstheme_m"}):SetTextColor(textCol)
			controlAdd("Checkbox", {Label = "Enable Dynamic Flashlights", Command = "vj_avp_flashlight"}):SetTextColor(textCol)
			Panel:AddControl("Label", {Text = "Note: Due to the way this code is handled, it is quite taxing on the game. Disable if you experience performance issues."}):SetTextColor(textCol)
		end)
	end)
end
...
if SERVER then
	...
	function VJ.AVP_ApplyRadiusDamage(attacker, inflictor, startPos, dmgRadius, dmgMax, dmgType, ignoreInnocents, realisticRadius, extraOptions, customFunc)
		...
		for _, v in ipairs((isnumber(extraOptions.UseConeDegree) and ents.FindInCone(startPos, extraOptions.UseConeDirection or attacker:GetForward(), dmgRadius, math.cos(math.rad(extraOptions.UseConeDegree or 90)))) or ents.FindInSphere(startPos, dmgRadius)) do
			...
			local nearestPos = startPos == attacker:GetPos() && VJ.GetNearestPositions(attacker, v) or v:NearestPoint(startPos) -- From the enemy position to the given position
