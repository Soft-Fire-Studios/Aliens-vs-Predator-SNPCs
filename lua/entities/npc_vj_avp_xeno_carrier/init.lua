...
-----------------------------------------------*/
ENT.Model = {"models/cpthazama/avp/xeno/carrier.mdl"} -- Model(s) to spawn with | Picks a random one if it's a table
ENT.StartHealth = 500
ENT.VJ_ID_Boss = true
...
function ENT:Breathe()
	if CurTime() > (self.NextBreathT or 0) then
		...
		VJ.CreateSound(self,snd,65,125)
		timer.Simple(1,function()
			if IsValid(self) then
				VJ.CreateSound(self,sndB,65,125)
			end
		end)
